import { POST } from '@/app/api/companies/[id]/offboard/route';
...
describe('cross-tenant IDOR - /api/companies/[id]/offboard', () => {
  beforeEach(() => jest.clearAllMocks());

  it('POST denies tenant B when targeting company A offboard', async () => {
    mockRequireApiPermission.mockResolvedValue(mockTenantBCrossTenantDenied());

    const response = await POST(
      makeGetRequest(`http://localhost/api/companies/${IDOR_COMPANY_A.id}/offboard`),
