...
export const toFieldJobSnapshotFromDayTicket = (
  ...
  ticketId: item.pendingSync && item.localJobId ? null : item.id,
  localJobId: options?.localJobId ?? item.localJobId ?? null,
  ...
  pendingSync: Boolean(options?.pendingSync ?? item.pendingSync),
});
